    ...
    prepare(sql) {
      return {
        bind(...args) {
          return {
            async first() {
              if (sql.includes("SELECT msg_id,result_status FROM idempotency")) {
...
test("Telegram webhook fails closed when secret is not configured", async () => {
  ...
  const response = await worker.fetch(
    telegramRequest(validUpdate, "anything"),
    env
  );
...
test("Telegram webhook rejects malformed JSON and invalid updates", async () => {
  ...
  const invalid = await worker.fetch(
    telegramRequest({ update_id: 9002 }, "expected-secret"),
    env
  );
...
test("valid Telegram update is queued exactly once and duplicate is idempotent", async () => {
  ...
  const first = await worker.fetch(
    telegramRequest(validUpdate, "expected-secret"),
    env
  );
